...
__webpack_require__.add({
    "../../node_modules/.pnpm/enhanced-resolve@5.24.5/node_modules/enhanced-resolve/lib/CachedInputFileSystem.js" (module, __unused_rspack_exports, __webpack_require__) {
        let firstCallback, firstResult, { nextTick } = __webpack_require__("process"), dirname = (path)=>{
        ...
        }, firstErr = null, dispatchQueue = [], dispatchQueueLength = 0, spareQueue = [], runDispatch = ()=>{
            let callback = firstCallback, err = firstErr, result = firstResult;
            if (firstCallback = void 0, firstErr = null, firstResult = void 0, 0 === dispatchQueueLength) return void callback(err, result);
            let queue = dispatchQueue, length = dispatchQueueLength;
            dispatchQueue = spareQueue, dispatchQueueLength = 0;
            let i = -3;
            try {
                for(callback(err, result), i = 0; i < length; i += 3)queue[i](queue[i + 1], queue[i + 2]);
            } finally{
                if ((i += 3) < length) if (void 0 === firstCallback) {
                    firstCallback = queue[i], firstErr = queue[i + 1], firstResult = queue[i + 2], nextTick(runDispatch);
                    for(let j = i + 3; j < length; j++)dispatchQueue[dispatchQueueLength++] = queue[j];
                } else {
                    let rest = queue.slice(i, length);
                    rest.push(firstCallback, firstErr, firstResult);
                    for(let j = 0; j < dispatchQueueLength; j++)rest.push(dispatchQueue[j]);
                    [firstCallback, firstErr, firstResult] = rest, dispatchQueueLength = (dispatchQueue = rest.slice(3)).length;
                }
                for(let j = 0; j < length; j++)queue[j] = void 0;
                queue.length > 1024 && (queue.length = 1024), spareQueue = queue;
            }
        }, scheduleDispatch = (callback, err, result)=>{
            if (void 0 === firstCallback) firstCallback = callback, firstErr = err, firstResult = result, nextTick(runDispatch);
            else {
                let queue = dispatchQueue;
                queue[dispatchQueueLength] = callback, queue[dispatchQueueLength + 1] = err, queue[dispatchQueueLength + 2] = result, dispatchQueueLength += 3;
            }
        };
        ...
        class CacheBackend {
            ...
            provide(path, options, callback) {
                ...
                if (void 0 !== cacheEntry) return cacheEntry.err ? scheduleDispatch(callback, cacheEntry.err, void 0) : scheduleDispatch(callback, null, cacheEntry.result);
    ...
    watchpack (module) {
        module.exports = __rspack_createRequire_require("../compiled/watchpack/index.js");
    },
    "./moduleFederationDefaultRuntime.js" (module) {
        module.exports = __rspack_createRequire_require("./moduleFederationDefaultRuntime.js");
    },
    "@rspack/binding" (module) {
        module.exports = __rspack_createRequire_require(process.env.RSPACK_BINDING || "@rspack/binding");
    },
    "node:crypto" (module) {
...
let binding_ = __webpack_require__("@rspack/binding");
var binding__default = __webpack_require__.n(binding_), AsyncDependenciesBlock = binding_.AsyncDependenciesBlock, ConcatenatedModule = binding_.ConcatenatedModule, ContextModule = binding_.ContextModule, Dependency = binding_.Dependency, EntryDependency = binding_.EntryDependency, ExternalModule = binding_.ExternalModule, Module = binding_.Module, NormalModule = binding_.NormalModule;
...
}), Object.defineProperty(binding_.Chunk.prototype, node_util.inspect.custom, {
    enumerable: !0,
    configurable: !0,
    value () {
        return {
...
}), Object.defineProperty(binding__default().Sources.prototype, 'get', {
    enumerable: !0,
    configurable: !0,
    value (sourceType) {
...
_computedKey = _to_property_key(binding__default().COMPILATION_HOOKS_MAP_SYMBOL);
class Compilation {
    #inner;
    #shutdown;
    #errors;
...
class CircularModulesInfoPlugin extends RspackBuiltinPlugin {
    name = binding_.BuiltinPluginName.CircularModulesInfoPlugin;
    raw(_compiler) {
        return createBuiltinPlugin(this.name, void 0);
    }
}
class CircularCheckRspackPlugin extends RspackBuiltinPlugin {
    name = binding_.BuiltinPluginName.CircularCheckRspackPlugin;
    _options;
    constructor(options = {}){
...
let CommonJsChunkFormatPlugin = base_create(binding_.BuiltinPluginName.CommonJsChunkFormatPlugin, ()=>{}), ContextReplacementPlugin = base_create(binding_.BuiltinPluginName.ContextReplacementPlugin, (resourceRegExp, newContentResource, newContentRecursive, newContentRegExp)=>{
...
}), CssChunkingPlugin = base_create(binding__default().BuiltinPluginName.CssChunkingPlugin, function(options = {}) {
    ...
    return options;
}), CssHttpExternalsRspackPlugin = base_create(binding_.BuiltinPluginName.CssHttpExternalsRspackPlugin, ()=>void 0), CssModulesPlugin = base_create(binding_.BuiltinPluginName.CssModulesPlugin, ()=>{}, 'compilation');
Object.defineProperty(binding__default().NormalModule.prototype, 'identifier', {
    enumerable: !0,
    configurable: !0,
    value () {
        return this[binding__default().MODULE_IDENTIFIER_SYMBOL];
    }
}), Object.defineProperty(binding__default().NormalModule.prototype, 'originalSource', {
    enumerable: !0,
    configurable: !0,
    value () {
...
}), Object.defineProperty(binding__default().NormalModule.prototype, 'emitFile', {
    enumerable: !0,
    configurable: !0,
    value (filename, source, assetInfo) {
        return this._emitFile(filename, SourceAdapter.toBinding(source), assetInfo);
    }
}), Object.defineProperty(binding__default().NormalModule, 'getCompilationHooks', {
    enumerable: !0,
    configurable: !0,
    value (compilation) {
        if (!(binding__default().COMPILATION_HOOKS_MAP_SYMBOL in compilation)) throw TypeError("The 'compilation' argument must be an instance of Compilation");
        let compilationHooksMap = compilation[binding__default().COMPILATION_HOOKS_MAP_SYMBOL], hooks = compilationHooksMap.get(compilation);
...
let utils_PLUGIN_NAME = 'css-extract-rspack-plugin', utils_pluginSymbol = Symbol.for(utils_PLUGIN_NAME), DEFAULT_FILENAME = '[name].css', LOADER_PATH = join(import.meta.dirname, 'cssExtractLoader.js');
class CssExtractRspackPlugin {
    static pluginName = utils_PLUGIN_NAME;
    ...
    apply(compiler) {
        compiler.hooks.thisCompilation.tap(utils_PLUGIN_NAME, (compilation)=>{
            binding_.NormalModule.getCompilationHooks(compilation).loader.tap(utils_PLUGIN_NAME, (loaderContext)=>{
                loaderContext[utils_pluginSymbol] = {
                    runtime: !1 !== this.options.runtime
                };
            });
        });
...
let DllEntryPlugin = base_create(binding_.BuiltinPluginName.DllEntryPlugin, (context, entries, options)=>({
        ...
        entries,
        name: options.name
    })), DllReferenceAgencyPlugin = base_create(binding_.BuiltinPluginName.DllReferenceAgencyPlugin, (options)=>options), lib_EntryOptionPlugin = class EntryOptionPlugin {
    apply(compiler) {
...
}, EntryPlugin = base_create(binding_.BuiltinPluginName.EntryPlugin, (context, entry, options = '')=>({
        context,
        entry,
...
let EnableWasmLoadingPlugin = base_create(binding_.BuiltinPluginName.EnableWasmLoadingPlugin, (type)=>type), EnsureChunkConditionsPlugin = base_create(binding_.BuiltinPluginName.EnsureChunkConditionsPlugin, ()=>{}), RemoveDuplicateModulesPlugin_RemoveDuplicateModulesPlugin = base_create(binding_.BuiltinPluginName.RemoveDuplicateModulesPlugin, ()=>({}));
base_create(binding_.BuiltinPluginName.EsmNodeTargetPlugin, ()=>void 0);
...
function getRawJavascriptParserOptions(parser) {
    ...
    return {
        ...
        importMeta: function(importMeta) {
            if ('boolean' == typeof importMeta) return String(importMeta);
            if ('object' == typeof importMeta && null !== importMeta) {
                let rawImportMeta = {};
                for (let [property, value] of Object.entries(importMeta))'boolean' == typeof value && (rawImportMeta[property] = value);
        ...
        worker: 'boolean' == typeof (worker = parser.worker) ? worker : Array.isArray(worker) ? {
            alias: worker
        } : worker,
...
let HashedModuleIdsPlugin = base_create(binding_.BuiltinPluginName.HashedModuleIdsPlugin, (options)=>({
        ...options
    }), 'compilation');
...
let HttpExternalsRspackPlugin = base_create(binding_.BuiltinPluginName.HttpExternalsRspackPlugin, (webAsync)=>({
        webAsync
    })), memoize = (fn)=>{
...
let compilationOptionsMap = new WeakMap(), hooks_compilationHooksMap = new WeakMap(), plugin_require = createRequire(import.meta.url), HTML_PLUGIN_UID = 0, HtmlRspackPlugin = base_create(binding_.BuiltinPluginName.HtmlRspackPlugin, function(c = {}) {
...
Object.defineProperty(binding__default().KnownBuildInfo.prototype, node_util.inspect.custom, {
    enumerable: !0,
    configurable: !0,
    value () {
        return {
...
}), Object.defineProperty(binding__default().KnownBuildInfo.prototype, 'assets', {
    enumerable: !0,
    configurable: !0,
    get () {
        if (this[binding__default().BUILD_INFO_ASSETS_SYMBOL][$assets]) return this[binding__default().BUILD_INFO_ASSETS_SYMBOL][$assets];
        let assets = new Proxy(Object.create(null), {
            ownKeys: ()=>this[binding__default().BUILD_INFO_ASSETS_SYMBOL].keys(),
            getOwnPropertyDescriptor: ()=>({
                    enumerable: !0,
                    configurable: !0
                })
        });
        return Object.defineProperty(this[binding__default().BUILD_INFO_ASSETS_SYMBOL], $assets, {
            enumerable: !1,
            configurable: !0,
            value: assets
        }), assets;
    }
}), Object.defineProperty(binding__default().KnownBuildInfo.prototype, 'fileDependencies', {
    enumerable: !0,
    configurable: !0,
    get () {
        return new Set(this[binding__default().BUILD_INFO_FILE_DEPENDENCIES_SYMBOL]);
    }
}), Object.defineProperty(binding__default().KnownBuildInfo.prototype, 'contextDependencies', {
    enumerable: !0,
    configurable: !0,
    get () {
        return new Set(this[binding__default().BUILD_INFO_CONTEXT_DEPENDENCIES_SYMBOL]);
    }
}), Object.defineProperty(binding__default().KnownBuildInfo.prototype, 'missingDependencies', {
    enumerable: !0,
    configurable: !0,
    get () {
        return new Set(this[binding__default().BUILD_INFO_MISSING_DEPENDENCIES_SYMBOL]);
    }
}), Object.defineProperty(binding__default().KnownBuildInfo.prototype, 'buildDependencies', {
    enumerable: !0,
    configurable: !0,
    get () {
        return new Set(this[binding__default().BUILD_INFO_BUILD_DEPENDENCIES_SYMBOL]);
    }
});
...
function loadLoader(loader, compiler, callback) {
    ...
    if ('module' === loader.type) try {
        ...
        if (void 0 === modulePromise) {
            void 0 === loadLoader_url && (loadLoader_url = __webpack_require__("node:url"));
...
async function runLoaders(compiler, context) {
    var buildInfo;
    let loaderState = context.loaderState, pitch = loaderState === binding_.JsLoaderState.Pitching, { resource } = context, traceData = JavaScriptTracer.isEnabled() ? {
    ...
    return traceData && JavaScriptTracer.endAsync({
        ...
        uuid: traceData.uuid,
        args: traceData.args
    }), compiler.options?.cache && Object.keys(buildInfo = context._module.buildInfo).some((key)=>!knownBuildInfoFields.has(key)) && buildInfo[binding__default().COMMIT_CUSTOM_FIELDS_SYMBOL](), context;
}
let JsLoaderRspackPlugin = base_create(binding_.BuiltinPluginName.JsLoaderRspackPlugin, (compiler)=>runLoaders.bind(null, compiler), 'thisCompilation'), JsonModulesPlugin = base_create(binding_.BuiltinPluginName.JsonModulesPlugin, ()=>{}, 'compilation'), LibManifestPlugin = base_create(binding_.BuiltinPluginName.LibManifestPlugin, (options)=>{
...
}), LightningCssMinimizerRspackPlugin = base_create(binding_.BuiltinPluginName.LightningCssMinimizerRspackPlugin, function(options) {
...
}), LimitChunkCountPlugin = base_create(binding_.BuiltinPluginName.LimitChunkCountPlugin, (options)=>options), BuiltinLazyCompilationPlugin = base_create(binding_.BuiltinPluginName.LazyCompilationPlugin, (currentActiveModules, entries, imports, client, reservedExternals, test)=>({
        imports,
        entries,
        test,
        client,
    ...
    if (compiler instanceof MultiCompiler) {
        ...
        for (let c of compiler.compilers){
            ...
            middlewareByCompiler.set(options.prefix, lazyCompilationMiddlewareInternal(c, activeModules, options.prefix)), applyPlugin(c, options, activeModules);
        }
        let keys = [
...
let NodeTargetPlugin = base_create(binding_.BuiltinPluginName.NodeTargetPlugin, ()=>void 0), NoEmitOnErrorsPlugin = base_create(binding_.BuiltinPluginName.NoEmitOnErrorsPlugin, ()=>void 0), NormalModuleReplacementPlugin = base_create(binding_.BuiltinPluginName.NormalModuleReplacementPlugin, (resourceRegExp, newResource)=>({
        ...
        handler: (percentage, msg, info)=>{
            ...
            key,
            value
        ]))), 'compilation'), RealContentHashPluginImpl = base_create(binding_.BuiltinPluginName.RealContentHashPlugin, ()=>{}, 'compilation'), RealContentHashPlugin_compilationHooksMap = new WeakMap();
RealContentHashPluginImpl.getCompilationHooks = (compilation)=>{
    checkCompilation(compilation);
    let hooks = RealContentHashPlugin_compilationHooksMap.get(compilation);
    return void 0 === hooks && (hooks = {
        updateHash: new SyncBailHook([
            'assets',
            'oldHash'
        ])
    }, RealContentHashPlugin_compilationHooksMap.set(compilation, hooks)), hooks;
};
let RemoveEmptyChunksPlugin = base_create(binding_.BuiltinPluginName.RemoveEmptyChunksPlugin, ()=>{}, 'compilation'), RsdoctorPluginImpl = base_create(binding_.BuiltinPluginName.RsdoctorPlugin, function(c = {
...
class SideEffectsFlagPlugin extends RspackBuiltinPlugin {
    ...
    name = binding_.BuiltinPluginName.SideEffectsFlagPlugin;
    affectedHooks = 'compilation';
...
let SwcJsMinimizerRspackPlugin = base_create(binding_.BuiltinPluginName.SwcJsMinimizerRspackPlugin, function(options) {
...
class SyncModuleIdsPlugin extends RspackBuiltinPlugin {
    options;
    name = binding_.BuiltinPluginName.SyncModuleIdsPlugin;
    affectedHooks = 'compilation';
    constructor(options){
        super(), this.options = options;
    }
...
let URLPlugin = base_create(binding_.BuiltinPluginName.URLPlugin, ()=>{}, 'compilation');
...
let configCache = {}, ES_VERSIONS_MAP = {
    ...
    [
        ...
        /^browserslist(?::(.+))?$/,
        (rest, context)=>{
            let inlineQuery = rest ? rest.trim() : null, browsers = binding__default().loadBrowserslist(inlineQuery, context);
...
You can also more options via the 'target' option: 'browserslist' / 'browserslist:env' / 'browserslist:query' / 'browserslist:path-to-config' / 'browserslist:path-to-config:env'`);
    ...
    if (!1 !== cache) switch(cache.type){
        ...
        case 'persistent':
            F(cache, 'name', ()=>{
                let cacheName = name ? `${name}-${mode}` : mode;
                return void 0 !== compilerIndex && compilerIndex > 0 ? `${cacheName}-${compilerIndex}` : cacheName;
            }), D(cache.storage, 'type', 'filesystem'), F(cache.storage, 'directory', ()=>node_path.resolve(context, 'node_modules/.cache/rspack')), F(cache.storage, 'location', ()=>node_path.resolve(cache.storage.directory, cache.name)), D(cache, 'version', ''), D(cache, 'maxAge', 604800), D(cache, 'maxVersions', 3), F(cache, 'buildDependencies', ()=>[]), F(cache.snapshot, 'immutablePaths', ()=>[]), F(cache.snapshot, 'unmanagedPaths', ()=>[]), F(cache.snapshot, 'managedPaths', ()=>[
                    /[\\/]node_modules[\\/][^.]/
    ...
    }), F(environment, 'logicalAssignment', ()=>tp && !0 === tp.logicalAssignment), F(environment, 'nodePrefixForCoreModules', ()=>{
        let v;
    ...
    }), D(output, 'bundlerInfo', {}), 'object' == typeof output.bundlerInfo && (D(output.bundlerInfo, 'version', "2.1.8"), D(output.bundlerInfo, 'bundler', 'rspack'), D(output.bundlerInfo, 'force', !1));
                ...
                case 'persistent':
                    {
                        ...
                        return {
                            ...
                            storage: nestedConfig(cache.storage, (storage)=>({
                                    type: storage.type,
                                    directory: optionalNestedConfig(storage.directory, (directory)=>node_path.resolve(context, directory)),
                                    location: optionalNestedConfig(storage.location, (location)=>node_path.resolve(context, location))
                                }))
                        };
                    }
                default:
...
class ResolverFactory {
    ...
    constructor(pnp, resolveOptions, loaderResolveOptions){
        this.#binding = new (binding__default()).JsResolverFactory(pnp, getRawResolve(resolveOptions), getRawResolve(loaderResolveOptions));
    }
...
class MultiStats {
    ...
    toJson(options) {
        ...
        }), childOptions.version && (obj.rspackVersion = "2.1.8", obj.version = "5.75.0"), childOptions.hash && (obj.hash = obj.children.map((j)=>j.hash).join(''));
        let mapError = (j, obj)=>({
                ...obj,
...
class NativeWatchFileSystem {
    ...
    watch(files, directories, missing, startTime, options, callback, callbackUndelayed) {
        ...
        return this.#watcher = watcher, nativeWatcher.watch(this.formatWatchDependencies(files), this.formatWatchDependencies(directories), this.formatWatchDependencies(missing), BigInt(startTime), (err, result)=>{
            ...
            close: ()=>{
                this.#inner === nativeWatcher && (this.#inner = void 0, this.#isFirstWatch = !0), nativeWatcher.close().catch((err)=>{
    ...
    getNativeWatcher(options) {
        ...
        }, nativeWatcher = new (binding__default()).NativeWatcher(nativeWatcherOptions);
...
}, CachedInputFileSystem = __webpack_require__("../../node_modules/.pnpm/enhanced-resolve@5.24.5/node_modules/enhanced-resolve/lib/CachedInputFileSystem.js");
            ...
            console: infrastructureLogging.console || function({ colors, appendOnly, stream }) {
                let currentStatusMessage, hasStatusMessage = !1, currentIndent = '', currentCollapsed = 0, clearStatusMessage = ()=>{
                    ...
                    stream.write(`\x1b[2K\r${coloredStr}`), hasStatusMessage = !0;
                        ...
                        })(__rspack_external_node_util_ac04460e.format(...args), prefix, colorPrefix, colorSuffix);
                        stream.write(`${str}\n`), writeStatusMessage();
...
let iterateConfig = (config, options, fn)=>{
    ...
    compilation: {
            ...
            if (logging || loggingDebug && loggingDebug.length > 0) {
                ...
                for (let [origin, logEntries] of compilationLogging){
                    ...
                    for (let entry of logEntries){
                        ...
                        message || (message = entry.args?.length ? __rspack_external_node_util_ac04460e.format(entry.args[0], ...entry.args.slice(1)) : '');
                        let newEntry = {
                            type,
                            message: message || '',
        ...
        version: (object)=>{
            object.version = "5.75.0", object.rspackVersion = "2.1.8";
        },
        ...
        entrypoints: (object, compilation, context, { entrypoints, chunkGroups, chunkGroupAuxiliary, chunkGroupChildren }, factory)=>{
            ...
            'auto' === entrypoints && !chunkGroups && (array.length > 5 || !chunkGroupChildren && array.every(({ chunkGroup })=>1 === chunkGroup.chunks.length && 1 === chunkGroup.assets.length && (!chunkGroupAuxiliary || chunkGroup.auxiliaryAssets?.length === 0))) || (object.entrypoints = factory.create(`${type}.entrypoints`, array, context));
        },
...
class RspackOptionsApply {
    process(options, compiler) {
        ...
        !enableLibSplitChunks && options.optimization.splitChunks && new SplitChunksPlugin(options.optimization.splitChunks).apply(compiler), options.optimization.removeEmptyChunks && new RemoveEmptyChunksPlugin().apply(compiler), options.optimization.realContentHash && new RealContentHashPluginImpl().apply(compiler);
...
Object.defineProperty(binding__default().Module.prototype, 'identifier', {
    enumerable: !0,
    configurable: !0,
    value () {
        return this[binding__default().MODULE_IDENTIFIER_SYMBOL];
    }
}), Object.defineProperty(binding__default().Module.prototype, 'originalSource', {
    enumerable: !0,
    configurable: !0,
    value () {
        let originalSource = this._originalSource();
...
}), Object.defineProperty(binding__default().Module.prototype, 'emitFile', {
    enumerable: !0,
    configurable: !0,
    value (filename, source, assetInfo) {
        return this._emitFile(filename, SourceAdapter.toBinding(source), assetInfo);
...
let CORE_VERSION = "2.1.8", VFILES_BY_COMPILER = new WeakMap();
...
let GET_COMPILER_ID = Symbol('getCompilerId');
class Compiler {
    #instance;
    #initial;
    #compilation;
    ...
    #getInstance(callback) {
        var output;
        let coreVersion, expectedCoreVersion, statsOptions, mode, experiments, error = CORE_VERSION === binding__default().EXPECTED_RSPACK_CORE_VERSION || CORE_VERSION.includes('canary') ? null : Error((coreVersion = CORE_VERSION, expectedCoreVersion = binding__default().EXPECTED_RSPACK_CORE_VERSION, process.env.RSPACK_BINDING ? `Unmatched version @rspack/core@${coreVersion} and binding version.
...
`));
            ...
            output: {
                ...
                environment: function(environment = {}) {
                    return {
                        ...
                        logicalAssignment: !!environment.logicalAssignment,
    ...
    #createHooksRegisters() {
        ...
        return {
            ...{
                registerCompilerThisCompilationTaps: createTap(binding__default().RegisterJsTapKind.CompilerThisCompilation, function() {
                ...
                registerCompilerCompilationTaps: createTap(binding__default().RegisterJsTapKind.CompilerCompilation, function() {
                ...
                registerCompilerMakeTaps: createTap(binding__default().RegisterJsTapKind.CompilerMake, function() {
                ...
                registerCompilerFinishMakeTaps: createTap(binding__default().RegisterJsTapKind.CompilerFinishMake, function() {
                ...
                registerCompilerShouldEmitTaps: createTap(binding__default().RegisterJsTapKind.CompilerShouldEmit, function() {
                ...
                registerCompilerEmitTaps: createTap(binding__default().RegisterJsTapKind.CompilerEmit, function() {
                ...
                registerCompilerAfterEmitTaps: createTap(binding__default().RegisterJsTapKind.CompilerAfterEmit, function() {
                ...
                registerCompilerAssetEmittedTaps: createTap(binding__default().RegisterJsTapKind.CompilerAssetEmitted, function() {
            ...
            ...{
                registerCompilationAdditionalTreeRuntimeRequirementsTaps: createTap(binding__default().RegisterJsTapKind.CompilationAdditionalTreeRuntimeRequirements, function() {
                ...
                registerCompilationRuntimeRequirementInTreeTaps: createMapTap(binding__default().RegisterJsTapKind.CompilationRuntimeRequirementInTree, function() {
                ...
                registerCompilationRuntimeModuleTaps: createTap(binding__default().RegisterJsTapKind.CompilationRuntimeModule, function() {
                ...
                registerCompilationBuildModuleTaps: createTap(binding__default().RegisterJsTapKind.CompilationBuildModule, function() {
                ...
                registerCompilationStillValidModuleTaps: createTap(binding__default().RegisterJsTapKind.CompilationStillValidModule, function() {
                ...
                registerCompilationSucceedModuleTaps: createTap(binding__default().RegisterJsTapKind.CompilationSucceedModule, function() {
                ...
                registerCompilationExecuteModuleTaps: createTap(binding__default().RegisterJsTapKind.CompilationExecuteModule, function() {
                ...
                registerCompilationFinishModulesTaps: createTap(binding__default().RegisterJsTapKind.CompilationFinishModules, function() {
                ...
                registerCompilationOptimizeModulesTaps: createTap(binding__default().RegisterJsTapKind.CompilationOptimizeModules, function() {
                ...
                registerCompilationAfterOptimizeModulesTaps: createTap(binding__default().RegisterJsTapKind.CompilationAfterOptimizeModules, function() {
                ...
                registerCompilationOptimizeTreeTaps: createTap(binding__default().RegisterJsTapKind.CompilationOptimizeTree, function() {
                ...
                registerCompilationOptimizeChunkModulesTaps: createTap(binding__default().RegisterJsTapKind.CompilationOptimizeChunkModules, function() {
                ...
                registerCompilationBeforeModuleIdsTaps: createTap(binding__default().RegisterJsTapKind.CompilationBeforeModuleIds, function() {
                ...
                registerCompilationChunkHashTaps: createTap(binding__default().RegisterJsTapKind.CompilationChunkHash, function() {
                ...
                registerCompilationChunkAssetTaps: createTap(binding__default().RegisterJsTapKind.CompilationChunkAsset, function() {
                ...
                registerCompilationProcessAssetsTaps: createTap(binding__default().RegisterJsTapKind.CompilationProcessAssets, function() {
                ...
                registerCompilationAfterProcessAssetsTaps: createTap(binding__default().RegisterJsTapKind.CompilationAfterProcessAssets, function() {
                ...
                registerCompilationSealTaps: createTap(binding__default().RegisterJsTapKind.CompilationSeal, function() {
                ...
                registerCompilationAfterSealTaps: createTap(binding__default().RegisterJsTapKind.CompilationAfterSeal, function() {
            ...
            ...{
                registerNormalModuleFactoryBeforeResolveTaps: createTap(binding__default().RegisterJsTapKind.NormalModuleFactoryBeforeResolve, function() {
                ...
                registerNormalModuleFactoryFactorizeTaps: createTap(binding__default().RegisterJsTapKind.NormalModuleFactoryFactorize, function() {
                ...
                registerNormalModuleFactoryResolveTaps: createTap(binding__default().RegisterJsTapKind.NormalModuleFactoryResolve, function() {
                ...
                registerNormalModuleFactoryResolveForSchemeTaps: createMapTap(binding__default().RegisterJsTapKind.NormalModuleFactoryResolveForScheme, function() {
                ...
                registerNormalModuleFactoryAfterResolveTaps: createTap(binding__default().RegisterJsTapKind.NormalModuleFactoryAfterResolve, function() {
                ...
                registerNormalModuleFactoryCreateModuleTaps: createTap(binding__default().RegisterJsTapKind.NormalModuleFactoryCreateModule, function() {
            ...
            ...{
                registerContextModuleFactoryBeforeResolveTaps: createTap(binding__default().RegisterJsTapKind.ContextModuleFactoryBeforeResolve, function() {
                ...
                registerContextModuleFactoryAfterResolveTaps: createTap(binding__default().RegisterJsTapKind.ContextModuleFactoryAfterResolve, function() {
            ...
            ...{
                registerJavascriptModulesChunkHashTaps: createTap(binding__default().RegisterJsTapKind.JavascriptModulesChunkHash, function() {
            ...
            ...(getOptions = (uid)=>((compilation, uid)=>{
                ...
                registerHtmlPluginBeforeAssetTagGenerationTaps: createTap(binding__default().RegisterJsTapKind.HtmlPluginBeforeAssetTagGeneration, function() {
                ...
                registerHtmlPluginAlterAssetTagsTaps: createTap(binding__default().RegisterJsTapKind.HtmlPluginAlterAssetTags, function() {
                ...
                registerHtmlPluginAlterAssetTagGroupsTaps: createTap(binding__default().RegisterJsTapKind.HtmlPluginAlterAssetTagGroups, function() {
                ...
                registerHtmlPluginAfterTemplateExecutionTaps: createTap(binding__default().RegisterJsTapKind.HtmlPluginAfterTemplateExecution, function() {
                ...
                registerHtmlPluginBeforeEmitTaps: createTap(binding__default().RegisterJsTapKind.HtmlPluginBeforeEmit, function() {
                ...
                registerHtmlPluginAfterEmitTaps: createTap(binding__default().RegisterJsTapKind.HtmlPluginAfterEmit, function() {
            ...
            ...{
                registerRealContentHashPluginUpdateHashTaps: createTap(binding_.RegisterJsTapKind.RealContentHashPluginUpdateHash, function() {
                    return RealContentHashPluginImpl.getCompilationHooks(getCompiler().__internal__get_compilation()).updateHash;
                }, function(queried) {
                    return function({ assets, oldHash }) {
                        return queried.call(assets, oldHash);
                    };
                })
            },
            ...{
                registerRuntimePluginCreateScriptTaps: createTap(binding__default().RegisterJsTapKind.RuntimePluginCreateScript, function() {
                ...
                registerRuntimePluginCreateLinkTaps: createTap(binding__default().RegisterJsTapKind.RuntimePluginCreateLink, function() {
                ...
                registerRuntimePluginLinkPreloadTaps: createTap(binding__default().RegisterJsTapKind.RuntimePluginLinkPreload, function() {
                ...
                registerRuntimePluginLinkPrefetchTaps: createTap(binding__default().RegisterJsTapKind.RuntimePluginLinkPrefetch, function() {
...
Object.defineProperty(binding__default().ConcatenatedModule.prototype, 'identifier', {
    enumerable: !0,
    configurable: !0,
    value () {
        return this[binding__default().MODULE_IDENTIFIER_SYMBOL];
    }
}), Object.defineProperty(binding__default().ConcatenatedModule.prototype, 'originalSource', {
    enumerable: !0,
    configurable: !0,
    value () {
        let originalSource = this._originalSource();
...
}), Object.defineProperty(binding__default().ConcatenatedModule.prototype, 'emitFile', {
    enumerable: !0,
    configurable: !0,
    value (filename, source, assetInfo) {
        return this._emitFile(filename, SourceAdapter.toBinding(source), assetInfo);
    }
}), Object.defineProperty(binding__default().ContextModule.prototype, 'identifier', {
    enumerable: !0,
    configurable: !0,
    value () {
        return this[binding__default().MODULE_IDENTIFIER_SYMBOL];
    }
}), Object.defineProperty(binding__default().ContextModule.prototype, 'originalSource', {
    enumerable: !0,
    configurable: !0,
    value () {
        let originalSource = this._originalSource();
...
}), Object.defineProperty(binding__default().ContextModule.prototype, 'emitFile', {
    enumerable: !0,
    configurable: !0,
    value (filename, source, assetInfo) {
        return this._emitFile(filename, SourceAdapter.toBinding(source), assetInfo);
    }
}), Object.defineProperty(binding__default().ExternalModule.prototype, 'identifier', {
    enumerable: !0,
    configurable: !0,
    value () {
        return this[binding__default().MODULE_IDENTIFIER_SYMBOL];
    }
}), Object.defineProperty(binding__default().ExternalModule.prototype, 'originalSource', {
    enumerable: !0,
    configurable: !0,
    value () {
        let originalSource = this._originalSource();
...
}), Object.defineProperty(binding__default().ExternalModule.prototype, 'emitFile', {
    enumerable: !0,
    configurable: !0,
    value (filename, source, assetInfo) {
        return this._emitFile(filename, SourceAdapter.toBinding(source), assetInfo);
...
let ModuleGraphConnection = binding_.ModuleGraphConnection;
Object.defineProperties(ModuleGraphConnection, {
    TRANSITIVE_ONLY: {
        value: binding__default().TRANSITIVE_ONLY_SYMBOL,
        enumerable: !0
    },
    CIRCULAR_CONNECTION: {
        value: binding__default().CIRCULAR_CONNECTION_SYMBOL,
        enumerable: !0
    }
});
...
let ModuleFederationRuntimePlugin = base_create(binding_.BuiltinPluginName.ModuleFederationRuntimePlugin, (options = {})=>options), ModuleFederationPlugin_require = createRequire(import.meta.url), MF_RUNTIME_LOADER = '@module-federation/runtime/rspack.js';
...
class ContainerReferencePlugin extends RspackBuiltinPlugin {
    ...
    raw(compiler) {
        let { remoteType, remotes } = this._options, bundlerName = 'rspack' === compiler.options.experiments.runtimeMode ? 'rspack' : 'webpack', remoteExternals = {}, importExternals = {};
        for (let [key, config] of remotes){
            let i = 0;
            for (let external of config.external){
                if (external.startsWith('internal ')) continue;
                let request = `${bundlerName}/container/reference/${key}${i ? `/fallback-${i}` : ''}`;
...
async function minify(source, options) {
    ...
    return binding__default().minify(source, _options);
}
async function transform(source, options) {
    let _options = JSON.stringify(options || {});
    return binding__default().transform(source, _options);
}
let exports_rspackVersion = "2.1.8", exports_version = "5.75.0", exports_WebpackError = Error, exports_config = {
    ...
    RealContentHashPlugin: RealContentHashPluginImpl,
    ...
    swc: {
        ...
        minifySync: function(source, options) {
            let _options = JSON.stringify(options || {});
            return binding__default().minifySync(source, _options);
        },
        transformSync: function(source, options) {
            let _options = JSON.stringify(options || {});
            return binding__default().transformSync(source, _options);
        }
    },
    resolver: {
