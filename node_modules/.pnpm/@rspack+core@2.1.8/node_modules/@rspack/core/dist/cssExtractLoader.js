...
let PLUGIN_NAME = 'css-extract-rspack-plugin', pluginSymbol = Symbol.for(PLUGIN_NAME), RELATIVE_PATH_REGEXP = /^\.\.?[/\\]/, BASE_URI = 'rspack-css-extract://', MODULE_TYPE = 'css/mini-extract', AUTO_PUBLIC_PATH = '__css_extract_public_path_auto__', ABSOLUTE_PUBLIC_PATH = `${BASE_URI}/css-extract-plugin/`, SINGLE_DOT_PATH_SEGMENT = '__css_extract_single_dot_path_segment__';
function hotLoader(content, context) {
    let localsJsonString = JSON.stringify(JSON.stringify(context.locals)), pluginData = context.loaderContext[pluginSymbol], changeSignal = pluginData?.runtime === !1 ? `\n        // ${Date.now()}` : '';
    return `${content}
    if(module.hot) {
      (function() {
