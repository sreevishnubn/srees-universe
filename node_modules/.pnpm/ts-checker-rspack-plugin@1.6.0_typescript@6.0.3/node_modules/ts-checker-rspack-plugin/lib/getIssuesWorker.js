...
function existsNormalized(normalizedPath) {
    const cached = existsCache.get(normalizedPath);
    if (void 0 !== cached) return cached;
    const result = external_node_fs_default().existsSync(normalizedPath);
...
function readStats(path) {
    ...
    if (!readStatsCache.has(normalizedPath)) {
        if (existsNormalized(normalizedPath)) readStatsCache.set(normalizedPath, external_node_fs_default().statSync(normalizedPath));
    }
...
function getRealPath(path) {
    ...
    if (!realPathCache.has(normalizedPath)) {
        ...
        while(base !== (0, external_node_path_namespaceObject.dirname)(base)){
            ...
            if (cachedBase) {
                realPathCache.set(normalizedPath, (0, external_node_path_namespaceObject.join)(cachedBase, nested));
                break;
            }
            if (existsNormalized(base)) {
                const realBase = normalizePath(realpath(base));
                realPathCache.set(base, realBase);
                realPathCache.set(normalizedPath, (0, external_node_path_namespaceObject.join)(realBase, nested));
                break;
            }
...
function fsRealPathHandlingLongPath(path) {
    return path.length < 260 ? external_node_fs_default().realpathSync.native(path) : external_node_fs_default().realpathSync(path);
}
const fsRealpath = external_node_fs_default().realpathSync.native ? 'win32' === process.platform ? fsRealPathHandlingLongPath : external_node_fs_default().realpathSync.native : external_node_fs_default().realpathSync;
...
function updateTimes(path, atime, mtime) {
    if (exists(path)) {
        const normalizedPath = normalizePath(path);
        external_node_fs_default().utimesSync(normalizedPath, atime, mtime);
        if (readStatsCache.has(normalizedPath)) readStatsCache.delete(normalizedPath);
    }
}
let hasChanges = false;
let mem;
const requireModule = (0, external_node_module_namespaceObject.createRequire)(__filename);
function getMemoryFileSystem() {
    if (mem) return mem;
    mem = requireModule('memfs').fs;
    return mem;
}
const memFileSystem = {
    ...realFileSystem,
...
function mem_file_system_readStats(path) {
    return mem_file_system_exists(path) ? getMemoryFileSystem().statSync(realFileSystem.normalizePath(path)) : void 0;
}
function mem_file_system_readFile(path, encoding) {
    const stats = mem_file_system_readStats(path);
    if (stats && stats.isFile()) return getMemoryFileSystem().readFileSync(realFileSystem.normalizePath(path), {
        encoding: encoding
    }).toString();
}
function mem_file_system_readDir(path) {
    const stats = mem_file_system_readStats(path);
    if (stats && stats.isDirectory()) return getMemoryFileSystem().readdirSync(realFileSystem.normalizePath(path), {
        withFileTypes: true
    });
    return [];
}
function mem_file_system_createDir(path) {
    hasChanges = true;
    getMemoryFileSystem().mkdirSync(realFileSystem.normalizePath(path), {
        recursive: true
    });
}
...
function mem_file_system_deleteFile(path) {
    hasChanges = true;
    if (mem_file_system_exists(path)) getMemoryFileSystem().unlinkSync(realFileSystem.normalizePath(path));
}
function mem_file_system_updateTimes(path, atime, mtime) {
    hasChanges = true;
    if (mem_file_system_exists(path)) getMemoryFileSystem().utimesSync(realFileSystem.normalizePath(path), atime, mtime);
}
...
function getReadFileSystem(path) {
    ...
    return 'readonly' !== mode || memFileSystem.hasChanges() ? passiveFileSystem : realFileSystem;
}
...
const getIssuesWorker = async (change, watching, defaultSeverity, withDependencies = false)=>{
    ...
    if (parseConfigIssues.length) return {
        issues: parseConfigIssues,
        dependencies: withDependencies ? getDependencies() : void 0
    };
