...
(()=>{
    ...
    function resolveTypeScriptGoPackageJsonPath(config) {
        if (!external_node_path_default().isAbsolute(config.typescriptPath) || 'package.json' !== external_node_path_default().basename(config.typescriptPath)) throw new Error("The typescriptPath option must be an absolute path to a package.json file when tsgo is enabled.");
        if (!config.tsgoPackage) throw new Error(`The typescriptPath option must point to "${TYPESCRIPT_PREVIEW_PACKAGE_JSON}" or "${TYPESCRIPT_PACKAGE}@>=7".`);
        return config.typescriptPath;
    }
    async function resolveTypeScriptGoNativeExecutablePath(packageJsonPath, packageName) {
        const getExePathPath = external_node_path_default().resolve(external_node_path_default().dirname(packageJsonPath), './lib/getExePath.js');
        const getExePathUrl = (0, external_node_url_namespaceObject.pathToFileURL)(getExePathPath).href;
        const getExePathModule = await import(getExePathUrl);
        const getExePath = getExePathModule.default || getExePathModule.getExePath;
        if ('function' != typeof getExePath) throw new Error(`Cannot resolve the native TypeScript executable from "${packageName}".`);
        const executablePath = getExePath();
        if ('string' != typeof executablePath || !external_node_fs_default().existsSync(executablePath)) throw new Error('Executable not found');
        return executablePath;
    }
    async function resolveTypeScriptGoBinPath(config) {
        const packageJsonPath = resolveTypeScriptGoPackageJsonPath(config);
        if ("typescript" === config.tsgoPackage) return resolveTypeScriptGoNativeExecutablePath(packageJsonPath, TYPESCRIPT_PACKAGE);
        return resolveTypeScriptGoNativeExecutablePath(packageJsonPath, TYPESCRIPT_PREVIEW_PACKAGE);
    }
    async function resolveTypeScriptGoExecutable(config) {
    ...
    function createTypeScriptGoArgs(config) {
        const args = config.build ? [
            '--build',
            config.configFile
        ] : [
            '--project',
    ...
    function logOutput(logger, output) {
    ...
    function getTypeScriptGoErrorCount(output) {
    ...
    function getTypeScriptGoIssueSeverity(severity, defaultSeverity) {
        return 'auto' === defaultSeverity ? severity : defaultSeverity;
    }
    function createTypeScriptGoIssue(message) {
        return {
            severity: 'error',
            code: "TSGO",
            message
    ...
    function getTypeScriptGoMarkerLength(output, startIndex) {
        const lines = output.slice(startIndex).split(/\r?\n/);
        for (const line of lines){
            if (diagnosticPattern.test(line) || globalDiagnosticPattern.test(line) || /^Found\s+\d+\s+errors?\b/.test(line)) break;
            const marker = line.match(/^\s*([~^]+)\s*$/);
            if (marker) return marker[1].length;
        }
    }
    function parseTypeScriptGoIssues(output, config, defaultSeverity = 'auto') {
        const issues = [];
        const plainOutput = stripAnsi(output);
        ...
        for (const match of plainOutput.matchAll(diagnosticPatternWithGlobal)){
            ...
            const markerLength = getTypeScriptGoMarkerLength(plainOutput, (match.index || 0) + match[0].length);
            const position = {
                line: Number(line),
                column: startColumn
            };
            issues.push({
                severity: getTypeScriptGoIssueSeverity(severity, defaultSeverity),
                code: `TS${code}`,
                message,
                file: external_node_path_default().isAbsolute(file) ? file : external_node_path_default().resolve(config.context, file),
                location: {
                    start: position,
                    end: {
                        line: position.line,
    ...
    function createTypeScriptGoExitIssues(output, signal, config, defaultSeverity = 'auto') {
        ...
        if (parsedIssues.length && (void 0 === errorCount || parsedIssues.length >= errorCount)) return parsedIssues;
        if (void 0 !== errorCount) return Array.from({
            length: Math.max(errorCount, 1)
        }, ()=>createTypeScriptGoIssue('tsgo check failed. See the output above for diagnostics.'));
        if (null !== signal) return [
            createTypeScriptGoIssue(`tsgo check interrupted by ${signal}. See the output above for diagnostics.`)
        ];
        return [
            createTypeScriptGoIssue('tsgo check failed. See the output above for diagnostics.')
        ];
    }
    function isTypeScriptGoIssue(issue) {
        return "TSGO" === issue.code;
    }
    function isTypeScriptGoStatsError(error) {
        const issue = error?.issue;
        if (issue && isTypeScriptGoIssue(issue)) return true;
        const message = error?.message;
        return 'string' == typeof message && message.includes("TSGO:");
    }
    async function runTypeScriptGo(config, logger, defaultSeverity = 'auto', signal) {
        AbortError.throwIfAborted(signal);
        ...
        return new Promise((resolve, reject)=>{
            let settled = false;
            let output = '';
            const childProcess = (0, external_node_child_process_namespaceObject.spawn)(executable.command, [
                ...executable.args,
                ...args
            ], {
                cwd: config.context,
            ...
            childProcess.on('close', (exitCode, exitSignal)=>{
                finish(()=>{
                    ...
                    if (issues.some(isTypeScriptGoIssue)) logOutput(logger, output);
                    resolve(issues);
                });
            });
        });
    }
    function getTypeScriptGoDependencies(config) {
        return {
            files: [
                config.configFile
            ],
            ...
            excluded: [
                external_node_path_default().join(config.context, 'node_modules')
            ],
            extensions: [
                '.ts',
                '.tsx',
    ...
    function tapDoneToAsyncGetIssues(compiler, config, state) {
        ...
        compiler.hooks.done.tap('TsCheckerRspackPlugin', async (stats)=>{
            ...
            if (visibleIssues.length) config.logger.error(visibleIssues.map((issue)=>formatter(issue)).join('\n'));
    ...
    function tapStartToRunWorkers(compiler, getIssuesWorker, getDependenciesWorker, config, state) {
        ...
        compiler.hooks.compilation.tap('TsCheckerRspackPlugin', async (compilation)=>{
            ...
            const workerResultPromise = (state.issuesPromise || Promise.resolve()).catch(()=>void 0).then(()=>{
                ...
                return issuesPool.submit(async ()=>{
                    try {
                        ...
                        const result = await getIssuesWorker(aggregatedFilesChange, state.watching, config.issue.defaultSeverity, reuseIssuesWorkerForDependencies);
    ...
    function readTsgoPackageJson(packageJsonPath) {
        return require(packageJsonPath);
    }
    function getTsgoPackage(packageJson) {
        if (packageJson.name === TYPESCRIPT_PACKAGE) {
            const versionMatch = packageJson.version?.match(/^(\d+)\.(\d+)(?:\.|$|-)/);
            if (versionMatch && Number(versionMatch[1]) >= 7) return "typescript";
        }
        if (packageJson.name === TYPESCRIPT_PREVIEW_PACKAGE) return 'preview';
    }
    function resolveTypeScriptGoPackage(packageJsonPath) {
        if (!external_node_path_default().isAbsolute(packageJsonPath) || 'package.json' !== external_node_path_default().basename(packageJsonPath)) return;
        try {
    ...
    function isDefaultTypeScriptGoPath(typescriptPath) {
        if (typescriptPath === TYPESCRIPT_PREVIEW_PACKAGE_JSON) return true;
        try {
            if (typescriptPath === require.resolve(TYPESCRIPT_PREVIEW_PACKAGE_JSON)) return true;
        } catch  {}
        if (!external_node_path_default().isAbsolute(typescriptPath) || 'package.json' !== external_node_path_default().basename(typescriptPath)) return false;
        try {
    ...
    function createTypeScriptGoSupportError(config, error) {
        const message = [
            `\`typescript.tsgo\` requires \`${TYPESCRIPT_PACKAGE}\` >= 7.0.0 or legacy \`${TYPESCRIPT_PREVIEW_PACKAGE}\`.`
        ];
        if (!isDefaultTypeScriptGoPath(config.typescriptPath)) message.push(`\`typescript.typescriptPath\` must be an absolute path to \`${TYPESCRIPT_PACKAGE_JSON}\` or \`${TYPESCRIPT_PREVIEW_PACKAGE_JSON}\`.`);
        if (error instanceof Error && error.message) message.push(`Failed to resolve the native TypeScript executable: ${error.message}`);
        return new Error(message.join(external_node_os_default().EOL));
    }
    function assertTypeScriptGoSupport(config) {
        try {
            const tsgoPackageJsonPath = resolveTypeScriptGoPackageJsonPath(config);
            const getExePathPath = external_node_path_default().resolve(external_node_path_default().dirname(tsgoPackageJsonPath), './lib/getExePath.js');
            if (!external_node_fs_default().existsSync(getExePathPath)) throw new Error();
        } catch (error) {
            throw createTypeScriptGoSupportError(config, error);
        }
    }
    async function assertTypeScriptGoExecutable(config) {
        try {
    ...
    function assertTypeScriptSupport(config) {
        if (config.tsgo) assertTypeScriptGoSupport(config);
        else {
            if ('package.json' === external_node_path_default().basename(config.typescriptPath)) throw new Error("When you use TsCheckerRspackPlugin without `typescript.tsgo`, `typescript.typescriptPath` should point to a path like `require.resolve('typescript')`.");
    ...
    function getStatsFilterHook(hook) {
        return hook;
    }
    function getStatsPrintHook(hook) {
        return hook;
    }
    function tapStartToRunTypeScriptGo(compiler, config, state) {
        const hooks = getPluginHooks(compiler);
        const { log, debug } = getInfrastructureLogger(compiler);
        let assertTypeScriptGoExecutablePromise;
        const assertTypeScriptGoExecutableOnce = async ()=>{
            try {
                if (!assertTypeScriptGoExecutablePromise) assertTypeScriptGoExecutablePromise = assertTypeScriptGoExecutable(config.typescript);
                await assertTypeScriptGoExecutablePromise;
            } catch (error) {
                assertTypeScriptGoExecutablePromise = void 0;
                throw error;
            }
        };
        ...
        compiler.hooks.compilation.tap('TsCheckerRspackPlugin', async (compilation)=>{
            ...
            compilation.hooks.statsFactory.tap('TsCheckerRspackPlugin', (stats)=>{
                ...
                errorsFilter.tap('TsCheckerRspackPlugin', (error)=>isTypeScriptGoStatsError(error) ? false : void 0);
                stats.hooks.result.for('compilation').tap('TsCheckerRspackPlugin', (statsCompilation, context)=>{
                    const hiddenErrorsCount = context.cachedGetErrors?.(context.compilation).filter(isTypeScriptGoStatsError).length || 0;
                    if (hiddenErrorsCount) Object.defineProperty(statsCompilation, hiddenTypeScriptGoErrors, {
            ...
            compilation.hooks.statsPrinter.tap('TsCheckerRspackPlugin', (stats)=>{
                const errorsInChildrenPrinter = getStatsPrintHook(stats.hooks.print.for('compilation.errorsInChildren!'));
                errorsInChildrenPrinter.tap('TsCheckerRspackPlugin', (_, { compilation, red })=>{
                    ...
                    if (!hiddenErrorsCount || compilation.children || !compilation.errorsCount || !compilation.errors) return;
                    const childErrorsCount = compilation.errorsCount - compilation.errors.length - hiddenErrorsCount;
                    if (childErrorsCount <= 0) return '';
                    const childErrorsLabel = 1 === childErrorsCount ? 'ERROR' : 'ERRORS';
                    const childErrorsMessage = `${childErrorsCount} ${childErrorsLabel} in child compilations (Use 'stats.children: true' resp. '--stats-children' for more details)`;
            ...
            } else log('Calling tsgo for single check.');
            ...
            state.issuesPromise = (state.issuesPromise || Promise.resolve()).catch(()=>void 0).then(()=>{
                ...
                return issuesPool.submit(async (signal)=>{
                    try {
                        ...
                        const issues = await runTypeScriptGo(config.typescript, config.logger, config.issue.defaultSeverity, signal);
                        if (state.aggregatedFilesChange === aggregatedFilesChange) state.aggregatedFilesChange = void 0;
                        if (state.abortController === abortController) state.abortController = void 0;
                        return issues;
                    } catch (error) {
        ...
        compiler.hooks.failed.tap('TsCheckerRspackPlugin', ()=>{
            if (!state.watching) abortTypeScriptGo();
        });
    }
    ...
    function resolveModule(request, resolveRoot) {
        return resolveRoot ? require.resolve(request, {
            paths: [
                resolveRoot
            ]
        }) : require.resolve(request);
    }
    function resolveInstalledTsgoPackage(resolveRoot) {
        try {
            ...
            if (tsgoPackage?.tsgoPackage === "typescript") return tsgoPackage;
        } catch  {}
    }
    function resolveDefaultPreviewPackageJsonPath(resolveRoot) {
        try {
    ...
    function resolveTypeScriptRuntimeConfig(options) {
        if (options.typescriptPath) {
            const tsgoPackage = resolveTypeScriptGoPackage(options.typescriptPath);
            const tsgo = void 0 === options.tsgo && tsgoPackage?.tsgoPackage === "typescript" ? true : options.tsgo;
            return {
                typescriptPath: options.typescriptPath,
                ...void 0 === tsgo ? {} : {
                    tsgo
                },
                ...true === tsgo && tsgoPackage ? {
                    tsgoPackage: tsgoPackage.tsgoPackage
                } : {}
            };
        }
        if (false === options.tsgo) return {
    ...
    function createTypeScriptWorkerConfig(compiler, options) {
        ...
        const resolveRoot = optionsAsObject.resolveRoot ? external_node_path_default().isAbsolute(optionsAsObject.resolveRoot) ? optionsAsObject.resolveRoot : external_node_path_default().resolve(compiler.options.context || process.cwd(), optionsAsObject.resolveRoot) : void 0;
        ...
        return {
            ...
            context: normalizedOptions.context || external_node_path_default().dirname(configFile),
            diagnosticOptions: {
                syntactic: false,
                semantic: true,
                declaration: false,
