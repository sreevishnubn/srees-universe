...
const route_workspace_changes_1 = require("./file-watching/route-workspace-changes");
...
const server = (0, net_1.createServer)(async (socket) => {
    ...
    socket.on('error', (e) => {
        logger_1.serverLogger.log(`Socket error: ${e.message}`);
...
async function handleMessage(socket, data) {
    ...
    if ((0, daemon_message_1.isDaemonMessage)(payload) && payload.env) {
        const changedEnvKeys = (0, daemon_environment_1.applyDaemonEnvFromClient)(payload.env);
        if (changedEnvKeys.length > 0) {
            logger_1.serverLogger.log(`Graph recompute necessary due to env variable refresh. Changed keys: ${changedEnvKeys.join(', ')}`);
    ...
    if (payload.type === 'PING') {
        ...
        await handleResult(socket, 'REQUEST_PROJECT_GRAPH', () => (0, handle_request_project_graph_1.handleRequestProjectGraph)(socket), mode);
    }
    ...
    else if ((0, configure_ai_agents_1.isHandleGetConfigureAiAgentsStatusMessage)(payload)) {
        await handleResult(socket, configure_ai_agents_1.GET_CONFIGURE_AI_AGENTS_STATUS, () => (0, handle_configure_ai_agents_1.handleGetConfigureAiAgentsStatus)(), mode);
    }
    else if ((0, configure_ai_agents_1.isHandleResetConfigureAiAgentsStatusMessage)(payload)) {
        await handleResult(socket, configure_ai_agents_1.RESET_CONFIGURE_AI_AGENTS_STATUS, () => (0, handle_configure_ai_agents_1.handleResetConfigureAiAgentsStatus)(), mode);
    }
    else {
        await (0, shutdown_utils_1.respondWithErrorAndExit)(socket, `Invalid payload from the client`, new Error(`Unsupported payload sent to daemon server: ${unparsedPayload}`));
    }
...
const handleWorkspaceChanges = async (err, changeEvents) => {
    ...
    try {
        ...
        (0, route_workspace_changes_1.routeWorkspaceChanges)(changeEvents);
    }
    catch (err) {
        ...
        try {
            server.listen(socketPath, async () => {
                try {
                    ...
                    (0, project_graph_incremental_recomputation_1.scheduleProjectGraphRecomputation)([], [], []);
                    ...
                    (0, handle_configure_ai_agents_1.handleGetConfigureAiAgentsStatus)().catch(() => {
                        // Ignore errors, this is a background operation
                    });
...
function forwardEnvToPluginWorkers(env) {
    (0, get_plugins_1.getPlugins)((0, nx_json_1.readNxJson)(workspace_root_1.workspaceRoot))
        .then((plugins) => {
