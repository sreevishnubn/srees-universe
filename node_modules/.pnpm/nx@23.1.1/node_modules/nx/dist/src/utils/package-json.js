...
function readTargetsFromPackageJson(packageJson, nxJson, projectRoot, workspaceRoot, packageManagerCommand) {
    ...
    for (const targetName in nx?.targets) {
        ...
        if (res[targetName] && (nxTarget.executor || nxTarget.command)) {
            ...
            res[targetName] = (0, target_merging_1.mergeTargetConfigurations)(nxTarget, res[targetName]);
        }
    }
    /**
    ...
    if (!isPrivate &&
        ...
        // array value forms).
        const nxReleasePublishTargetDefaults = (0, project_configuration_utils_1.readTargetDefaultsForTarget)('nx-release-publish', nxJson?.targetDefaults, '@nx/js:release-publish') ?? {};
        res['nx-release-publish'] = {
