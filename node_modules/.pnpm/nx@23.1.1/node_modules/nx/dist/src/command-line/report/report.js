...
async function reportHandler() {
    ...
    packageVersionsWeCareAbout.forEach((p) => {
        bodyLines.push(`${pc.green(p.package.padEnd(padding))} : ${pc.bold(p.version)}`);
    });
    if (nxKey) {
        bodyLines.push('');
        ...
        for (const powerpackPlugin of powerpackPlugins) {
            bodyLines.push(`${pc.green(powerpackPlugin.name.padEnd(padding))} : ${pc.bold(powerpackPlugin.version)}`);
        }
        bodyLines.push('');
    }
    else if (nxKeyError) {
    ...
    if (communityPlugins.length) {
        ...
        communityPlugins.forEach((p) => {
            bodyLines.push(`${pc.green(p.name.padEnd(padding))}: ${pc.bold(p.version)}`);
        });
    }
    ...
    if (mismatchedNxVersions && mismatchedNxVersions.length > 0) {
        ...
        for (const { version, chain } of mismatchedNxVersions) {
            if (chain.length === 0) {
                ...
                bodyLines.push(`  - ${chain.reverse().join(' → ')} → ${pc.bold(`nx@${version}`)}`);
            }
        }
        bodyLines.push('');
