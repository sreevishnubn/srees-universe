    ...
    for (const commit of commits) {
        ...
        if (scopePatterns.length > 0) {
            ...
            for (const pattern of scopePatterns) {
                ...
                const inGroupMatches = perPatternMatches.filter((p) => releaseGroupProjectSet.has(p));
            ...
            scopedProjects = new Set(matches.filter((p) => releaseGroupProjectSet.has(p)));
        }
