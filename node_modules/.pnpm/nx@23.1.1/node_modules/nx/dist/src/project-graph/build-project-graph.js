...
async function updateProjectGraphWithPlugins(context, initProjectGraph, plugins, sourceMap) {
    ...
    let spinner;
    const inProgressPlugins = new Set(createDependencyPlugins.map((plugin) => plugin.name));
    const getSpinnerText = () => spinner
        ? (0, plugin_progress_text_1.formatPluginProgressText)('Creating project graph dependencies', inProgressPlugins)
        : '';
...
async function applyProjectMetadata(graph, plugins, context, sourceMap) {
    ...
    let spinner;
    const createMetadataPlugins = plugins.filter((plugin) => plugin.createMetadata);
    const inProgressPlugins = new Set(createMetadataPlugins.map((p) => p.name));
    const getSpinnerText = () => spinner
        ? (0, plugin_progress_text_1.formatPluginProgressText)('Creating project metadata', inProgressPlugins)
        : '';
    spinner = createMetadataPlugins.length
        ? new delayed_spinner_1.DelayedSpinner(getSpinnerText(), {
            progressTopic: progress_topics_1.ProgressTopics.GraphConstruction,
        })
        : undefined;
